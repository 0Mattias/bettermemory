...
- **Tamper evidence: detect-only, single-machine, files only.** Since 7.3.0 every store write records the SHA-256 of the bytes it put on disk beside the memory's index row, a rebuild carries a recorded hash forward for a file no pull brought down whose bytes no longer match it, and `bettermemory doctor` (`memory_content_evidence`) names the files whose content changed with no store write behind the change — a hand edit, a script writing into the directory, a pull done outside `bettermemory sync`. It is evidence, not protection: a writer with access to the index file can restamp or delete it, there is no MAC or chain over the files or the event log, a record that arrived by `sync pull` is anchored at the pull (a remote edit is the remote host's to notice), and a forged trust field in a file the tool itself last wrote is still read as written. Treat the store directory's integrity as equal to the host account's integrity. The integrity benchmark measures the remaining gap: a file planted around the write API reads `unaccounted` on every surface, and the same file with a forged `write` event line reads `local` (`docs/eval-results.md`).
...
- **A stamp names what it checked.** Since 7.3.0 `memory_verify` refuses to stamp a memory whose cited paths resolve when the call attests none of them and declares no claims; the refusal lists the paths to attest. A hand-written `last_verified_at` in the file is still read as written — tamper evidence is the open item above — but a stamp the tool itself lands on a checkable memory now carries the attestation the read side's drift legs run on.
- **What is not defended.** A pulled body that is merely false or manipulative, with no secret in it, is admitted; it reads `synced` and unverified, and it is retrievable. Git history is trusted exactly as git trusts it: there is no signature verification and no `fsck`. A remote deletion removes the file here, with `git log` in the store directory as the only record. Tombstones under `.tombstones/` sync without admission and remain on the roadmap; since 7.3.0 a restore re-runs the claims oracle and the attestation existence check and drops what fails, clearing the stamp with it. A store cloned by hand rather than pulled carries no pull record, so its files read `untracked` and the remote-stamp rule does not apply to them.
