...
1. **MCP server registration** ([`.mcp.json`](.mcp.json)) — spawns `uvx bettermemory` as a stdio MCP server. All 22 tools (18 `memory_*` + 4 `episode_*`) become available on plugin enable.
...
You should see the 22 tools listed with the `mcp__bettermemory__` prefix (18 `memory_*` + 4 `episode_*`). Then:
