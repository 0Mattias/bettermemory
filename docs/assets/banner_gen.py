...
def icosphere_levels(max_level):
    ...
    for lvl in range(max_level + 1):
        ...
        def mid(a, b):
            ...
            if key not in cache:
                va, vb = vs[a], vs[b]
                vs.append(norm(((va[0] + vb[0]) / 2, (va[1] + vb[1]) / 2,
                                (va[2] + vb[2]) / 2)))
                cache[key] = len(vs) - 1
            return cache[key]
        nf = []
...
def render_sphere(cx, cy, radius, rot, strength=1.0, glints=True):
    ...
    for (w, op), segs in sorted(groups.items()):
        parts.append(f'<path d="{"".join(segs)}" stroke="{INK}" '
    ...
    for lvl, rf, r_dot, op_f, op_b in ((0, 1.035, 2.3, 0.55, 0.13),
        ...
        for v in verts:
            ...
            if z > 0.08 and op_f:
                parts.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="{r_dot}" '
                             f'fill="{INK}" opacity="{op_f * strength:.2f}"/>')
            elif z <= 0.08 and op_b:
                parts.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="{r_dot * 0.7:.1f}" '
...
def qpoint(p0, c, p1, t):
    mt = 1 - t
    return (mt * mt * p0[0] + 2 * t * mt * c[0] + t * t * p1[0],
...
def spindle(bx, by, rb, ax, ay, ra):
    ...
    for i in range(n):
        ...
        ctrl = ((p0[0] + p1[0]) / 2 + (RND.random() - 0.5) * 26,
                midy + ((by + ay) / 2 - midy) * pinch)
        op = 0.11 + 0.20 * math.cos(th) ** 2
        parts.append(f'<path d="M{p0[0]:.1f} {p0[1]:.1f}Q{ctrl[0]:.1f} '
    ...
    for idx, t_ghost, t_solid in ((2, 0.22, 0.77), (4, 0.30, 0.72), (6, 0.18, 0.79)):
        ...
        parts.append(f'<circle cx="{gx:.1f}" cy="{gy:.1f}" r="1.8" '
                     f'fill="{INK}" opacity="0.26"/>')
        parts.append(f'<circle cx="{sx:.1f}" cy="{sy:.1f}" r="2.2" '
                     f'fill="{INK}" opacity="0.66"/>')
        parts.append(f'<circle cx="{sx:.1f}" cy="{sy:.1f}" r="4.4" '
                     f'fill="none" stroke="{INK}" stroke-width="0.55" opacity="0.30"/>')
    mx, my = qpoint(*threads[4], 0.5)
    parts.append(f'<circle cx="{mx:.1f}" cy="{my:.1f}" r="1.9" '
...
def build():
    ...
    svg.append('<!-- bettermemory. Artwork in the anaphase light system; '
               'wordmark outlines set in Bitstream Charter, '
               '(c) 1989-1992 Bitstream Inc., freely licensed. -->')
    svg.append('<defs>')
    svg.append(f'<clipPath id="card"><rect x="0" y="0" width="{W}" height="{H}" '
               f'rx="14"/></clipPath>')
    for gid, cx, cy, r in (("glowA", ax_c, ay_c, ar * 1.85),
                           ("glowB", bx_c, by_c, br * 1.7)):
        svg.append(f'<radialGradient id="{gid}" cx="50%" cy="50%" r="50%">'
                   ...
                   f'</radialGradient>')
    svg.append('<linearGradient id="vig" x1="0" y1="0" x2="0" y2="1">'
               ...
               '</linearGradient>')
    svg.append('<filter id="grain" x="0" y="0" width="100%" height="100%">'
    ...
    svg.append(f'<ellipse cx="{(ax_c + bx_c) / 2:.0f}" cy="200" rx="560" ry="240" '
               f'fill="#ffffff" opacity="0.16"/>')
    svg.append(f'<circle cx="{ax_c}" cy="{ay_c}" r="{ar * 1.85:.0f}" fill="url(#glowA)"/>')
    svg.append(f'<circle cx="{bx_c}" cy="{by_c}" r="{br * 1.7:.0f}" fill="url(#glowB)" '
               f'opacity="0.7"/>')
    ...
    svg.append(f'<g transform="translate(96 212)"><path d="{wordmark_d}" '
               f'fill="{INK}"/></g>')
    svg.append(f'<g transform="translate(98 258)"><path d="{tagline_d}" '
    ...
    svg.append('</g>')
    svg.append(f'<rect x="0.5" y="0.5" width="{W - 1}" height="{H - 1}" rx="13.5" '
