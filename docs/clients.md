...
The manual path also skips the plugin's **hooks**, and those carry real behavior on Claude Code: the Stop hook (`bettermemory hook stop`) settles retrievals at turn end and runs the silent-miss audit, the UserPromptSubmit hook (`bettermemory hook prompt`) delivers score-gated proactive recall, and the SessionStart hook (`bettermemory hook session-start`) prints the per-scope counts; each is one HTTP call to the local daemon the stdio server already runs in front of. Without the Stop hook the usage telemetry is one-sided: the health report suppresses its dead-weight and cold-endorsement verdicts rather than misreading the silence (the honesty gate). Wire them from the plugin's `hooks/hooks.json` shapes if you want the full loop on a manual install; on other hosts there is no hook surface, and the in-process settlement plus the honesty gate are the designed degradation.
