...
The core is built and battle-hardened. The differentiated surface is live. After a run of correctness sweeps, 3.7.0 rotated back to feature work — execute-the-curation, onboarding, retrieval-quality levers, and the eval driver that unblocks the comparative — 3.8.0 added the write-time credential check that keeps secrets out of the plain-text store, and 3.9.0 swept the path-drift false-signal classes and added the `verified_absent_paths` attestation plus the linked-worktree auto-scope fix.
...
- **Staleness verdict on every retrieval** — calendar age + filesystem path drift + git commit drift folded into one `staleness_verdict`, with the inline `path_drift = {checked, missing, verified, expected_absent}` list and the `verified_absent_paths` attestation for paths that are absent on purpose (remote-host, other-platform, cited-as-not-here).
...
- **Hardened** — whole-codebase correctness audits (3.3.4, 3.5.0, 3.6.5), concurrent multi-agent store access (3.2.0), and cross-platform (incl. Windows) fixes, each landed with a regression test. ~2,100 tests, 80% coverage floor, Python 3.11–3.14, MIT.

## Recently shipped

The CHANGELOG has the release-by-release detail; the arc, by theme:

- **Drift signals you can trust (3.9.0).** A 4-round multi-agent hunt (224 agents, every finding adversarially re-verified with a runnable repro) over the heuristic-extraction surfaces, triggered by live phantom `path_drift_missing` flags in the dogfood store. All 20 confirmed path-extraction findings fixed (spaced macOS/Windows segments, URL routes, `file.py:407` citations, `$HOME/` spelling, glob/template shapes, single-arg commands, attestation/ambiguity interactions), plus two HIGHs: a credential-gate false negative (`password is X.` read as a module ref) and the linked-worktree auto-scope blackout (spawned-agent worktrees couldn't see the primary checkout's memories). The dogfood store went from 13 phantom flags to zero; the 146 remaining verified findings are parked in `docs/audit/` as a pre-verified queue.
