...
Returns `{pending, pending_total}` — each pending row is `{id, a, b, similarity, method, detector}` with `a` / `b` as `{id, body, scopes, updated}` — plus the `scan` counters key in scan mode and a `resolved` echo (`status` of `"confirmed"` / `"dismissed"` / `"already_resolved"`) in resolve mode. The `resolved` echo carries `link_written` (bool) on a contradiction verdict and `links_cleared` (a list of `{source, target}`, empty when there was nothing to clear) on a compatible one.
