...
- `mode: str | None = None`. Ranker — `"hybrid"` (default since 2.6.8: RRF fusion of keyword + BM25, plus a semantic leg whenever an embeddings extra is installed — installing it is the whole opt-in, no config flag; with no extra it degrades gracefully to keyword+BM25 fusion), `"keyword"` (legacy TF + coverage + recency; no IDF, weaker on rare-term queries), `"bm25"` (Okapi BM25 with the same scope-bonus + recency), or `"semantic"` (sentence-transformers cosine; needs an embeddings extra, and the extra alone is now sufficient — it previously errored with the install hint even when installed, because the model resolved only under `semantic_dedup = true`, which contradicted this very sentence. `semantic_dedup` governs WRITE-time dedup only and no longer gates retrieval). Per-call override beats `[behavior] search_mode` for ranker selection. Use `"keyword"` for literal identifiers / file paths if you need byte-stable 1.6.0 ranking; otherwise hybrid is a strict improvement. The fused hybrid score lives in a smaller scale (~0.01–0.05) than single-ranker scores — compare across modes via `relevance`, not raw `score`. When `[behavior] endorsement_boost` is on (off by default), a bounded usage-aware factor (≤ +10%, same ceiling as the recency boost) additionally nudges memories the model has *explicitly* applied up the ranking — a near-tie breaker that never overrides relevance. Two sibling flags, also off by default, ride the same near-tie band: `outcome_demotion` slides memories carrying still-active `ignored` / `contradicted` outcomes down (floored at -15%, `contradicted` weighted double — a negative stops counting once it ages past the 30-day window, a later non-auto `applied` supersedes it, or a `memory_update` / `memory_verify` postdates it), and `corroboration_boost` nudges memories whose persisted `corroborations` rollup has accumulated (same +10% ceiling and saturating shape as endorsement, read off the record so it costs no event-log walk). None of the three can override relevance; with all three off no factor multiplies and the shipped default ranking is unchanged.
