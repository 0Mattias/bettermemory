...
`316781e`. The skip now runs through `semantic_setup._semantic_rank_leg_active`, which requires all three conditions. Mode comparison is left unnormalised on purpose, mirroring the handler: a mis-cased `"Hybrid"` gets no model there, so it must read as no-leg here.
...
- Related code: `_check_retrieval_discrimination` in `src/bettermemory/doctor.py`; `_semantic_rank_leg_active`, `_semantic_model_configured` and `_search_mode_needs_model` in `src/bettermemory/semantic_setup.py`.
- Related incidents: [`2026-07-26-staleness-verdict-constant-function.md`](2026-07-26-staleness-verdict-constant-function.md) — the same failure class one day later, and measured rather than reasoned about.
