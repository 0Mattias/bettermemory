...
**Escape hatch for pre-fix events.** When a fix lands that invalidates a batch of historical misses (e.g. the v2.7.3 cwd-suppression change), `bettermemory consolidate --acknowledge-misses-before <ISO_TS>` writes one additive `silent_miss_cutoff` event with `cutoff_ts=<ISO_TS>`. Every rate-reporting surface — `memory_health`, `memory_scope_overview`, and the `bettermemory eval` CLI (`compute_eval`) — drops any `turn_audited` *or* `search_miss` events earlier than the cutoff, invalidating both numerator and denominator so the rate isn't skewed and the surfaces agree over the same event stream. Each honors the latest cutoff seen; an earlier cutoff is ignored. Reversible by a later cutoff or by pruning the event manually. The per-event companion, `memory_acknowledge_miss(event_id, reason)`, retracts one false-positive miss the same way on every surface: the referenced `search_miss` drops from the numerator while the audited denominator keeps its turn (the audit itself wasn't wrong — the flagged verdict was). A third numerator-only filter follows the same cross-surface contract: a miss whose top-hit memory has since been **tombstoned** is no longer actionable and drops from the numerator (never the audited denominator) on every surface — `memory_health` always applied this; `compute_eval` takes an optional `tombstoned_ids` set (the CLI passes the store's real tombstone set; the default `None` keeps the old behavior for callers without a store handle, like the comparative harness).
...
**Invalidated telemetry — a deliberate asymmetry.** The sweep honors the bulk `silent_miss_cutoff` escape hatch exactly as the rate surfaces do (latest `cutoff_ts` wins, resolved globally even when the cutoff's own timestamp falls outside `--since`): cutoff-invalidated misses were flagged by a since-fixed code bug, not by a genuine rule decision, so replaying them would pollute the "is v1 over-firing" calibration with noise no rule change can address. They drop from `Replayable misses` and the legacy-skip footnote alike, as if never logged. Per-event `miss_ack` retractions are deliberately **not** honored here — an acked miss is a *confirmed false positive of the current rule*, which is exactly the signal a stricter candidate is judged against ("would v2/v3/v4 have declined to flag the miss a human had to retract?"). The rate surfaces drop acked misses because they report *outstanding actionable misses*; the sweep replays *rule decisions*.
