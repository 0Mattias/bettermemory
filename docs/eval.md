...
- `memory_helped_rate` and `endorsement_rate` come back as `n/a` from the offline `BetterMemoryAdapter`, not `0.0`. They require a live agent emitting `record_use` events with claim excerpts; deriving them from the gold labels would just relabel recall, so the offline adapter reports `n/a` by construction. **The live-agent *driver* has landed** (`tests/eval/driver.py`, `python -m tests.eval.comparative --driver scripted|live`): it runs the real ranker for retrieval, asks an `Agent` to decide which retrieved memory it cited, emits genuinely-shaped `search` + `use` events, and feeds them through `compute_eval` so all three rates compute. Two agents implement the protocol: a deterministic `ScriptedAgent` (a recorded transcript — authored citations that prove the compute path end-to-end and give CI a reproducible trio, **not** a published measurement; its `memory_helped_rate` sits below recall precisely because it cites only some of what it retrieves), and a `LiveAgent` that drives a real model (the publishable measurement, gated behind `ANTHROPIC_API_KEY`). So the open piece before publication is no longer the *driver* — it is running the `LiveAgent` (and the competitor runs below).
