...
# ---------------------------------------------------------------------------
# Skip counts — unconditional keys, so a backup checker can assert on them
# ---------------------------------------------------------------------------
...
def test_export_no_tombstones_reports_null_not_zero_for_that_half(
    ...
    """--no-tombstones never reads the tombstone half, so 0 there would
    assert an absence nobody checked. `null` says "not examined",
    ...
    assert parsed["skipped_active_files"] == 0
    # The unread tombstone half must not be described on stderr.
