...
Hand-written per-surface copies of one command string — ``doctor``'s
fix hints plus two surfaces in the pre-4.0 semantic lane — are the
drift that shipped an unquoted, zsh-refused install command: each
surface got fixed on its own schedule, and each fix closed an instance
rather than the class. ``bettermemory._install_hints`` closed the class
by owning the atoms every surface composes from. This module holds both
halves of that deal:
...
2. the spellings appear nowhere else in shipped source, so a new
...
def test_composed_forms_keep_doctors_exact_shipped_shape() -> None:
    """The two convenience forms keep the shape `doctor` shipped.
