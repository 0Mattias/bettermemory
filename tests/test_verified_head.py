...
through a tombstone and held on the store's row, independent of the
...
def _row_anchor(store: Store, memory_id: str) -> str | None:
    """The anchor as the store's own row holds it."""
...
def test_the_stamp_records_the_anchor_and_the_row_carries_it(
    memory_dir: Path,
) -> None:
    store = Store(memory_dir)
    memory = _seed(store)
    ...
    assert _row_anchor(store, memory.id) == _SHA
    # A fresh Store reads the same record back: the round trip is the
    # row, not a cached object.
...
def test_a_stamp_with_no_anchor_clears_the_previous_one(memory_dir: Path) -> None:
    ...
    assert _row_anchor(store, memory.id) is None
...
def test_a_hand_written_anchor_that_is_not_a_hash_reads_as_none(
    ...
    """The loader drops what the store would have refused, so an edit
    of the row cannot smuggle a revision string to git."""
    store = Store(memory_dir)
    memory = _seed(store)
    with store.conn:
        store.conn.execute(
            "UPDATE memories SET verified_head = 'main' WHERE id = ?", (memory.id,)
        )
...
def test_a_tombstone_carries_the_anchor_and_a_restore_can_drop_it(
    ...
    dropped = store.restore_trimmed(memory.id, drop_verified_head=True)
...
def test_the_row_follows_the_anchor_and_the_log_replays_it(
    memory_dir: Path,
) -> None:
    store = Store(memory_dir)
    memory = _seed(store)
    ...
    assert _row_anchor(store, memory.id) == _OTHER
    store.mark_verified(memory.id)
    assert _row_anchor(store, memory.id) is None
    store.mark_verified(memory.id, verified_head=_SHA)
    # Replaying the log produces the same row, anchor included.
    assert store.refold()["tables"]["memories"]["unaccounted"] == []
    assert _row_anchor(store, memory.id) == _SHA
