...
def server_with_events(memory_dir: Path) -> tuple[Any, Store, SessionState]:
    """A live server whose recorder writes into the store's log."""
...
async def test_write_duplicate_emits_event(
    ...
    events = [e for e in _events(store) if e["kind"] == "write"]
...
async def test_write_force_records_forced_true(
    ...
    forced_events = [
        e for e in _events(store) if e["kind"] == "write" and e.get("forced") is True
    ]
...
async def test_search_records_query_and_returned_ids(
    ...
    search_events = [e for e in _events(store) if e["kind"] == "search"]
...
async def test_show_emits_event(
    ...
    show_events = [e for e in _events(store) if e["kind"] == "show"]
...
async def test_update_records_fields_changed(
    ...
    upd = [e for e in _events(store) if e["kind"] == "update"]
    assert len(upd) == 1
...
async def test_remove_records_reason(
    ...
    rm = [e for e in _events(store) if e["kind"] == "remove"]
    assert len(rm) == 1
...
async def test_scope_disable_enable_record_events(
    ...
    server, store, _ = server_with_events
    await _call(server, "memory_admin", action="disable_scope", scope="projects:foo")
    await _call(server, "memory_admin", action="enable_scope", scope="projects:foo")

    kinds = [e["kind"] for e in _events(store)]
