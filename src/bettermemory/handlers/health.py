...
DESC_MEMORY_HEALTH = (
    ...
    "- `cross_repo_drift` — the estate check: the same drift for "
    "memories anchored in OTHER repos, judged read-only at their "
    "recorded worktrees (capped; unresolvable groups listed as "
    "skipped with reasons). The unanchorable count carries verified "
    "records whose origin names no repo or worktree, so nothing could "
    "be resolved for them — coverage this check did not have, not a "
    "clean bill.\n"
    ...
    "- `actor_slices` — per-actor pivot `{declared, undeclared}` "
    "keyed on the declared `client` (what memory_search / "
    "memory_list filter on), each entry `{client, memories, "
    "memories_in_window, events, searches, applies, models, "
    "principals}`. `models` / `principals` are the census of "
    "spellings that exist to filter on; an empty dict means none "
    "declared. `undeclared` is always present, including at zero, "
    "and holds every record whose writer named no client — the "
    "majority on any store with pre-7.10.0 history. Memory and "
    "event counts reconcile exactly against "
    "`total_active_memories` / `total_events`.\n"
