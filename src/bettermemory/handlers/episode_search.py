...
DESC_EPISODE_SEARCH = (
    ...
    "most-recent-`max_results` window: when matches exceed the cap "
    "it keeps the MOST-RECENT N, oldest-first inside that window — "
    "'what did I conclude across the last few sessions?' reads the "
    "tail, not the head.\n\n"
    ...
    "checkout) and, deliberately, when the recorded worktree is gone "
    "from disk or you are in a LINKED worktree of the checkout that "
    "wrote it — so under agent fan-out the primary checkout's episodes "
    "stay visible. An EXPLICIT `swarm_id` or `parent_session_id` is "
    "exempt and never worktree-filtered: naming a cohort or session is "
    "deliberate cross-worktree intent. Set `auto_scope=False` to sweep "
    "the bare walk across worktrees too.\n\n"
    ...
    "the caller's git worktree (see WORKTREE SCOPING for the four "
    "cases it lets through; explicit swarm_id / parent_session_id "
    "reads are never filtered). Set False to sweep the bare walk "
    "across every worktree sharing the root.\n"
