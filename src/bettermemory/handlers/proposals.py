...
def accept_proposal(
    *,
    ...
    3. Run the SHARED content-gate chain (``CONTENT_GATES``) — the same
       ...
       Every category commits on accept, ``user-inference`` included, from
       the MCP tool and the CLI alike. The extractor stamps first-person
       preferences ``user-inference``, and those used to stage a pending
       write from a session, so a stated preference waited on a
       confirmation round trip before it landed; ``memory_write`` no
       longer stages that category, so neither does this. The label rides
       on the record, which is what keeps the inference distinguishable
       and correctable.
