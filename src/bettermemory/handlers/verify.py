...
DESC_MEMORY_VERIFY = (
    ...
    "Concurrency: same optimistic-concurrency CAS as memory_update "
    "(see it), but the stakes are attestation-specific — "
    "`verified_*` lists have REPLACE (not append) semantics, so two "
    "parallel verifies attesting different paths would lose one. On "
    'a losing write the response is `status="stale"`; re-fetch with '
    "memory_show, reassess your attestation against the now-current "
    "verified_* lists, and retry. Contract is reread + reattest, "
    "not silent merge.\n\n"
    "Parameters:\n"
    "- `id`: memory id.\n"
