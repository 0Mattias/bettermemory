...
async def memory_audit_turn(
    ...
    # ignored/contradicted ones down — plus the recency half-life.
    # Threading only the endorsement half was a telemetry-honesty bug in
    # both directions, because the miss verdict reads ONLY the rank-1
    # hit: a memory production had demoted out of the top slot still
    # held rank 1 here, and the hit production's demotion promoted
    # instead was never the one this probe judged.
    #
