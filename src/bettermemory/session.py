...
SSE), each client needs its own state, otherwise client A's
`disabled_scopes` would silently bleed into client B's searches. The `SessionRegistry`
...
class SessionState:
    ...
    Nothing here is durable: disabled scopes, use-tokens and the turn
    counter are all per-process by design and reset when the server
    restarts.
    """

    session_id: str = field(default_factory=_new_session_id)
    # Which `SessionRegistry` bucket this state serves. Left at the
    # default for a bare `SessionState`, which is what the stdio
    ...
    # settled them. Stashed rather than deleted so the loss is
    # observable: drained by `pop_expired_use_tokens()` so the handler
    # layer can emit one `use_token_expired` event per batch. One map
    # and no second-TTL bookkeeping: the model never hands a token back
    # (`memory_record_use` takes `memory_ids`), so the drain is the
    # whole lifetime.
    ...
    def advance_turn(self) -> int:
        ...
        ages against. Also evicts wall-clock-expired use-tokens, stashing
        rather than deleting them (`_expired_use_tokens`) so the handler
        layer can drain them and emit one `use_token_expired` event per
        drop.
        """
        self.turn_counter += 1
    ...
    def pop_expired_use_tokens(self) -> list[PendingUseToken]:
        ...
        telemetry-enabled check. That ordering is what keeps the stash
    ...
    def reset(self) -> None:
        ...
        # session state (disabled scopes), not to "rotate" the
...
class SessionRegistry:
    ...
    def for_request(self, ctx: _Ctx | None) -> SessionState:
        ...
        # other (losing disabled_scopes / turn_counter).
