...
def add_subparser(
    ...
    """Register the ``migrate`` subparser (with the ``origin`` and ``v8``
    sub-subparsers)."""
    help_text = (
        "One-shot data migrations. `migrate v8` reads a v8 store directory "
        "into a bettermemory 9 store; `migrate origin` backfills the origin "
        "field on memories written before that field existed."
    )
...
def _cli_migrate_v8(
    *,
    ...
    from ..sqlite_store import STORE_FILENAME
