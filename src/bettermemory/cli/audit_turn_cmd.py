...
def add_subparser(
    ...
    help_text = (
        ...
        "calls `audit.probe_for_miss` over the pool `memory_search` "
        "would have built for that message "
        "(`handlers.search.resolve_search_pool`) — above the FTS "
        "index threshold a capped, query-biased prefilter slice, not "
        "the whole store. That pool is drawn with the raw user "
        "message rather than the query the model would have typed, "
