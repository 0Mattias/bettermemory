...
DESC_MEMORY_SCOPE_OVERVIEW = (
    ...
    "global). Returns `{current_repo, current_cwd, auto_scope, "
    "scopes: {scope: count}, total, disabled_scopes, "
    "curation_pending: {stale, never_verified, drifted, cold, "
    "dead, silent_misses, endorsement_debt}}` — `curation_pending` "
    "is an integer-count rollup the model should branch on at "
    "session start: any non-zero `dead` or `drifted` is a cue to "
    "suggest a curation pass when the conversation has time, and "
    "non-zero `silent_misses` / `endorsement_debt` hint that the "
    "telemetry surface has actionable backlog. Use this tool once "
    "at the start of a conversation to see whether stored memory "
    "exists for the current project — if `total` is 0, you can "
    "skip memory_search for the rest of the session unless the "
    "user explicitly asks. If the count is non-zero, memory_search "
    "remains the way to retrieve content; this tool only tells "
    "you whether searching is likely to be fruitful. Set "
    "`auto_scope=False` to count across all stored memory "
    "regardless of origin (the cross-project view). Counts respect "
    "session-disabled scopes."
)
